...
test.describe('Activities - CRUD Operations', () => {
  ...
  test('should show validation error when required fields are empty', async ({ page }) => {
    ...
    if (canProceed) {
      ...
      const errorVisible = await page.locator('[data-error], .error, [role="alert"]').isVisible().catch(() => false);
  ...
  test('should delete activity', async ({ page }) => {
    ...
    if (await deleteButton.isVisible()) {
      ...
      await activitiesPage.goto(groupId);
      const isVisible = await page.getByText(activityData.temat || '').isVisible().catch(() => false);
  ...
  test('should search activities by name', async ({ page }) => {
    ...
    if (hasSearch) {
      ...
      const otherVisible = await page.getByText(otherActivity.temat || '').isVisible().catch(() => false);
