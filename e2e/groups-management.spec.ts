...
test.describe('Groups Management', () => {
  ...
  test('should edit group settings', async ({ page }) => {
    ...
    const settingsButton = page.locator('main').getByRole('button', { name: /ustawienia|edytuj/i }).first();
  ...
  test('should delete group as owner', async ({ page }) => {
    ...
    if (await deleteButton.isVisible()) {
      ...
      const isVisible = await page.getByText(groupData.name).isVisible().catch(() => false);
