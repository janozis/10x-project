...
export function GroupMembersView({ groupId }: GroupMembersViewProps): JSX.Element {
  ...
  const [confirmState, setConfirmState] = React.useState<{ open: boolean; member?: MemberRowVM; busy?: boolean }>({ open: false });

  React.useEffect(() => {
    if (error) toast.error(error);
  }, [error]);

  const handleChangeRole = React.useCallback(async (row: MemberRowVM, newRole: GroupRole) => {
    try {
  ...
  const handlePromote = React.useCallback(async (row: MemberRowVM) => {
    try {
  ...
  return (
    <section className="space-y-4" aria-labelledby="members-heading">
      <header className="flex items-center justify-between">
        <h1 id="members-heading" className="text-xl font-semibold">Członkowie grupy</h1>
