...
const EmptyStateComponent = ({ onOpenCreate, onOpenJoin, variant = "default" }: EmptyStateProps): JSX.Element => {
  ...
  return (
    <div
      className="flex flex-col items-center justify-center gap-4 rounded-lg border bg-background p-10 text-center"
      data-test-id="groups-empty-state"
    >
      ...
      <div>
        <h2 className="text-lg font-medium" data-test-id="groups-empty-state-message">
          {isDeleted ? "Brak usuniętych grup" : "Brak grup"}
        </h2>
      ...
      <div className="flex items-center gap-2">
        <Button variant="outline" onClick={onOpenJoin} data-test-id="groups-empty-join-button">
          Dołącz do grupy
        </Button>
        <Button onClick={onOpenCreate} data-test-id="groups-empty-create-button">
          Utwórz grupę
        </Button>
      </div>
    </div>
