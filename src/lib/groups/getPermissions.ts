...
export async function getGroupPermissionsResource(
  groupId: UUID,
  init?: RequestInit,
  baseUrl?: URL | string
): Promise<ResourceResult<GroupPermissionsDTO>> {
  const url = baseUrl ? new URL(`/api/groups/${groupId}/permissions`, baseUrl) : `/api/groups/${groupId}/permissions`;
  ...
  if (!response.ok) {
    ...
    return {
      ok: false,
      status,
      error: apiError ?? fallbackError(status, "Nie udało się pobrać uprawnień użytkownika."),
    } satisfies ResourceFailure;
  }

  const parsed = parseSingle<GroupPermissionsDTO>(body);
  if (!parsed) {
    return {
      ok: false,
      status: 500,
  ...
  return {
    ...
    status,
    data: parsed.data,
  } satisfies ResourceSuccess<GroupPermissionsDTO>;
}
