import type { ActivityStatus, ActivityWithEditorsDTO, ApiListResponse, ApiResponse, UUID, ActivityEditorDTO, AIEvaluationDTO } from "@/types";
...
export function listActivities(
  groupId: UUID,
  options?: { status?: ActivityStatus; assigned?: "me"; search?: string; deleted?: "only"; limit?: number; cursor?: string }
...
export function patchActivity(activityId: UUID, payload: ActivityUpdateInput): Promise<ApiResponse<ActivityWithEditorsDTO>> {
...
export function deleteActivity(activityId: UUID): Promise<ApiResponse<{ id: UUID; deleted_at: string }>> {
  return fetchJson<ApiResponse<{ id: UUID; deleted_at: string }>>(`/api/activities/${activityId}`, { method: "DELETE" });
}

export function restoreActivity(activityId: UUID): Promise<ApiResponse<ActivityWithEditorsDTO>> {
  return fetchJson<ApiResponse<ActivityWithEditorsDTO>>(`/api/activities/${activityId}/restore`, { method: "POST", body: JSON.stringify({}) });
}
...
export function requestActivityAIEvaluation(activityId: UUID): Promise<ApiResponse<{ queued: true; next_poll_after_sec: number }>> {
  return fetchJson<ApiResponse<{ queued: true; next_poll_after_sec: number }>>(`/api/activities/${activityId}/ai-evaluations`, {
    method: "POST",
    body: JSON.stringify({}),
  });
}
